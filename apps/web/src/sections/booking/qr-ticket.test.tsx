...
describe('QrTicket', () => {
  it('renders the QR code for the token', async () => {
    render(<QrTicket token={token} reference={reference} expiresAt={expiresAt} now={new Date('2026-08-10T09:00:00Z')} />);
  ...
  it('warns and withholds the code once the token has expired', async () => {
    render(<QrTicket token={token} reference={reference} expiresAt={expiresAt} now={new Date('2026-08-11T07:00:00Z')} />);
